...
name: bid-toolkit
description: |
  招投标标书自动化工具链（擎标开源核心 / v4.0）。当用户需要：标书框架搭建与锁定、
...
metadata:
  version: 4.0.0
  display_name: 擎标 · 标书自动化工具链
  tags: [tender, 标书, 招投标, 排版, 质检, 脱敏, 审标, RAG, AI PC, 擎标]
  license: Apache-2.0
---

# 擎标 · bid-toolkit 标书自动化工具链（v4.0）
...
| **框架搭建** | `orchestrate init` / `parse 招标文件` / `framework --template property` | 解析并建框架 |
...
| **其他** | `table` 两列表格 / `watermark` 水印 / `rfp` 招标文件生成 | |
